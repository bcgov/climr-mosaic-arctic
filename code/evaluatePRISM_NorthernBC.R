...
pilotarea <- ext(c(-132.75, -123, 56, 59))
  ...
  # for(e in 1:2){
    ...
    for(m in c(1,3,7,10)){
      ...
      if(source=="prism"){
        ...
        clim <- crop(clim, pilotarea) #crop to a smaller test area
        values(clim)[!is.finite(values(clim))] <- NA
      } else if(source=="usask"){
        # USask wrf
        dir <- "//objectstore2.nrs.bcgov/ffec/Climatologies/USask_WRF/monthly_clim_regridded/"
        clim <- rast(paste(dir, list.files(dir, pattern=paste(month.abb.lowercase[m], c("tmin", "tmax", "prec")[e], sep="_")), sep=""))
      ...
      par(mfrow=c(1,1))
      png(filename=paste("results/NorthernBC_lapserates_residuals_", source, "_", elements[e], "_", monthcodes[m], ".png", sep=""), type="cairo", units="in", width=8, height=9, pointsize=14, res=600)
      ...
      for(i in 1:dim(point.look)[1]){
        ...
        axis(2, at=pretty(clim.look), labels=paste(pretty(clim.look), "%", sep=""))
      ...
      par(mfrow=c(1,1))
      png(filename=paste("results/NorthernBC_lapserates_raw_", source, "_", elements[e], "_", monthcodes[m], ".png", sep=""), type="cairo", units="in", width=8, height=9, pointsize=14, res=600)
      ...
      for(i in 1:dim(point.look)[1]){
        ...
        unit <- if(elements[e] != "Pr") "K" else "%"
        mtext(paste0("lapse =", round(100*(2^(lm$coefficients[2])-1)*1000, 1), " ", unit, "/km"), col="blue", adj=0.05, line = -1.5, side=1, cex = 0.6)
        lm <- lm(clim.look~dem.look)
        if(is.finite(lm$coefficients[2])) abline(lm)
        mtext(paste0("lapse =", round(100*(2^(lm$coefficients[2])-1)*1000, 1), " ", unit, "/km"), adj=0.05, line = -2.5, side=1, cex = 0.6)
        mtext(i, adj=0.95, line = -2.5, side=3, cex=1.5, font=2)
      }
