...
dem.akbcus <- extend(dem.bc, prism) # start with the BC DEM and extend it to the full study area range
...
for(e in 2:length(elements)){
  for(m in 1:length(monthcodes)){
    ...
    dir <- paste("C:/Users/CMAHONY/OneDrive - Government of BC/Data/PRISM_akbcus/", elements[e], "/", sep="")
    ...
    dir <- "C:/Users/CMAHONY/OneDrive - Government of BC/Data/daymet/"
    ...
    data.train <- cbind(predictand.points, points, elev.points, daymet.points, coastal.points)
    colnames(data.train) <- c("predictand", "x","y","elev","daymet", "coastal")
    data.train <- data.train[!is.na(data.train$elev),]
    data.train <- data.train[!is.na(data.train$daymet),]
    # str(data.train)
    
    # train the model
    model.akbcus <- ranger(y=data.train[,1], x=data.train[,-1], num.trees=300, replace = F, sample.fraction = 0.25, oob.error=F)
    ...
    data.fillarea <- cbind(points.fillarea,elev.fillarea,daymet.fillarea, coastal.fillarea)
    colnames(data.fillarea) <- c("x","y","elev","daymet", "coastal")
    ...
    # write the raster
    writeRaster(clim.pred, paste("outputs/composite_WNA_1981_2010_", elements[e], monthcodes[m], ".tif", sep=""), overwrite=T)
